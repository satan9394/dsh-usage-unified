...
function resolveBounds(input) {
	if (input.from !== void 0) return {
		from: input.from,
		to: input.to ?? input.today
	};
	if (input.range === "all" || input.range === "custom") return {
		from: "",
		to: input.today
	};
...
var UnifiedIndexStore = class {
	...
	*/
	snapshot(query) {
		...
		const custom = query.from !== void 0;
		...
		const rangeId = custom ? "custom" : query.range;
		const snapshotQuery = {
			from: bounds.from,
			to: bounds.to,
...
function parseCommon(url) {
	...
	if (rawTo !== void 0 && rawFrom === void 0) throw new Error("A to date needs a from date");
	...
	if (rawFrom !== void 0) parsed.from = rawFrom;
	if (rawTo !== void 0) parsed.to = rawTo;
	return parsed;
}
