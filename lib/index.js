...
function collectCalls(input, query) {
	...
	const all = query.range === "all";
	const rows = [];
	for (const fold of folds) for (const call of Object.values(fold.calls)) {
		if (!all) {
...
async function readArtifact(path, fromCursor, cachedHeader) {
	...
	return decodeArtifactBytes(fromCursor > 0 ? bytes.subarray(fromCursor) : bytes, path.endsWith(".zstd"), fromCursor, cachedHeader);
}
//#endregion
	...
	async scan() {
		...
		try {
			...
			for (const home of discovery.homes) for await (const artifact of walkSessionArtifacts(home)) {
				...
				const unchanged = previous !== void 0 && previous.size === artifact.size && previous.mtimeMs === artifact.mtimeMs && previous.ino === artifact.ino && previous.dev === artifact.dev;
				if (unchanged) {
					processed += 1;
					publishProgress();
					continue;
