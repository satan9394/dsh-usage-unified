window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function withCustom(params, custom) {
			if (custom !== void 0) {
				params.set("from", custom.from);
				if (custom.to !== void 0) params.set("to", custom.to);
			}
			return params;
		}
		/** Fetch the dashboard snapshot for one window. */
		async function fetchSnapshot(range, scope, workspace, signal, custom) {
		...
		function exportUrl(range, scope, workspace, format, custom) {
...
`;
		...
		function Card({ icon, label, value, detail, accent, hero, spark }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("article", {
				className: "us-card",
				"data-hero": hero ? "true" : void 0,
				style: accent === void 0 ? void 0 : { "--us-accent-card": accent },
				children: [
					hero && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "us-sheen",
						"aria-hidden": "true"
					}),
		...
		function formatCost(value, numberLocale) {
			const digits = value >= 1e3 ? 0 : value >= 10 ? 1 : 2;
		...
		var PanelStore = class {
			...
			isOpen = (id, fallback) => this.state.get(id) ?? fallback;
			toggle = (id, fallback) => {
				this.state.set(id, !this.isOpen(id, fallback));
				this.emit();
			};
			open = (id) => {
		...
		*/
		function Panel({ id, title, note, className, children, defaultOpen = true }) {
			const { t } = useLocale();
			const open = (0, react.useSyncExternalStore)(panels.subscribe, () => panels.isOpen(id, defaultOpen));
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: className === void 0 ? "us-panel" : `us-panel ${className}`,
				"data-collapsed": open ? void 0 : "true",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "us-panel-head",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
						className: "us-panel-toggle",
						"aria-expanded": open,
						"aria-controls": `us-panel-${id}`,
						title: open ? t("collapsePanel") : t("expandPanel"),
						onClick: () => panels.toggle(id, defaultOpen),
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
							className: "us-chevron",
							viewBox: "0 0 16 16",
							"aria-hidden": "true",
		...
		*/
		function gapFillDays(raw, max = 400) {
			if (raw.length < 2) return raw;
			const first = raw[0].date;
			const last = raw[raw.length - 1].date;
			const byDate = new Map(raw.map((day) => [day.date, day]));
			const out = [];
			const cursor = /* @__PURE__ */ new Date(`${first}T00:00:00Z`);
			const end = /* @__PURE__ */ new Date(`${last}T00:00:00Z`);
		...
		function SmoothTrend({ snapshot, note }) {
			const { t, lang, numberLocale } = useLocale();
			const [off, setOff] = (0, react.useState)({});
			const [hover, setHover] = (0, react.useState)(null);
			const [tipAt, setTipAt] = (0, react.useState)(null);
			const hourly = snapshot.range.id === "1d";
			const points = (0, react.useMemo)(() => {
				if (hourly) return snapshot.hours.map((bucket, hour) => ({
					key: String(hour),
			...
			const series = [{
				...
				label: t("trendTotal"),
				color: "#922bff",
				get: (point) => point.tokens
			}, ...hourly ? [] : [
				{
					id: "input",
					label: t("input"),
					color: "#1684ff",
					get: (point) => point.input
				},
				{
					id: "output",
					label: t("output"),
					color: "#219653",
					get: (point) => point.output
				},
				{
					id: "cache",
					label: t("cacheRead"),
					color: "#f59e0b",
					get: (point) => point.cacheRead
				}
			]];
			const max = Math.max(1, ...points.map((point) => point.tokens));
			const xAt = (index) => padL + (points.length <= 1 ? plotW / 2 : index / (points.length - 1) * plotW);
			const yAt = (value) => 350 - value / max * plotH;
			const smooth = (get) => {
				if (points.length === 0) return "";
				const pts = points.map((point, index) => [xAt(index), yAt(get(point))]);
			...
			const areaPath = totalPath === "" ? "" : `${totalPath} L ${xAt(points.length - 1).toFixed(1)} ${350 .toFixed(1)} L ${xAt(0).toFixed(1)} ${350 .toFixed(1)} Z`;
			...
			const hoverPoint = hover === null ? void 0 : points[hover];
			const onMove = (event) => {
				const rect = event.currentTarget.getBoundingClientRect();
				if (rect.width === 0 || points.length === 0 || false) return;
				const frac = Math.min(1, Math.max(0, (event.clientX - rect.left) / rect.width));
				const index = Math.round(frac * (points.length - 1));
				const value = points[index]?.tokens ?? 0;
				setHover(index);
				setTipAt({
					x: rect.left + (xAt(index) - padL) / plotW * rect.width,
					y: rect.top + (yAt(value) - padT) / plotH * rect.height
				});
			};
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Panel, {
				...
				className: "us-trend",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "us-trend-chart",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
						className: "us-trend-svg",
						viewBox: `0 0 ${W} ${H}`,
						role: "img",
						...
						children: [
							...
							[
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("g", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("line", {
									...
									y: y + 4,
									textAnchor: "end",
									children: compact(max * (4 - i) / 4, numberLocale)
								})] }, i);
							}),
							ticks.map((index) => {
							...
							areaPath !== "" && !off.total && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
								d: areaPath,
								fill: "url(#us-total-fill)"
							}),
							series.map((item) => off[item.id] ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
								d: smooth(item.get),
								fill: "none",
								stroke: item.color,
								strokeWidth: item.id === "total" ? 2.4 : 1.7,
								strokeLinecap: "round",
								strokeLinejoin: "round"
							}, item.id)),
							hover !== null && hoverPoint !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("line", {
								className: "us-trend-hover-line",
								x1: xAt(hover),
								y1: padT,
								x2: xAt(hover),
								y2: 350
							}), series.map((item) => off[item.id] ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("circle", {
								cx: xAt(hover),
								cy: yAt(item.get(hoverPoint)),
								r: 3.4,
								fill: item.color
							}, item.id))] }),
					...
					}), hover !== null && hoverPoint !== void 0 && tipAt !== null && (0, react_dom.createPortal)(/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						"data-usage-stats": true,
						className: "us-trend-tip",
						style: {
							position: "fixed",
							left: Math.min(window.innerWidth - 96, Math.max(96, tipAt.x)),
							top: tipAt.y,
							transform: tipAt.y > 180 ? "translate(-50%, -112%)" : "translate(-50%, 16%)"
						},
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: hoverPoint.label }), series.map((item) => off[item.id] ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("i", { style: { background: item.color } }), item.label] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: compact(item.get(hoverPoint), numberLocale) })] }, item.id))]
					}), document.body)]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "us-trend-legend",
					children: series.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
						style: { "--us-series": item.color },
						"aria-pressed": !off[item.id],
						onClick: () => setOff((current) => ({
							...current,
							[item.id]: !current[item.id]
						})),
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("i", {}), item.label]
					}, item.id))
				})]
			});
		}
		...
		*/
		function ModelPanel({ snapshot }) {
			...
			const legend = /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "us-bucket-legend",
				children: BUCKETS.map((bucket) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("i", {
					className: "us-bucket-key",
					"data-bucket": bucket
				}), t(BUCKET_LABEL[bucket])] }, bucket))
			});
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Panel, {
				id: "models",
				title: t("modelUsage"),
				note: legend,
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						className: "us-model-list",
						children: [
							models.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "us-model-meta",
								children: t("noData")
							}),
							rows.map((model) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "us-model-row",
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: "us-model-line",
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											...
											" ",
											t("callsCount"),
											model.costUsd !== void 0 && ` · ≈$${formatCost(model.costUsd, numberLocale)}`
										]
									})
								]
							}, model.key)),
							models.length > MODEL_ROW_LIMIT && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "us-model-more",
								onClick: () => setExpanded((current) => !current),
								children: expanded ? t("showLess") : t("showAllModels", { n: models.length })
							})
						]
					})]
				})
		...
		*/
		function SessionRanking({ snapshot, onSelect }) {
			const { t, numberLocale } = useLocale();
			const [expanded, setExpanded] = (0, react.useState)(false);
			const rows = expanded ? snapshot.sessions : snapshot.sessions.slice(0, SESSION_ROW_LIMIT);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Panel, {
				id: "sessions",
				title: t("sessionRanking"),
				note: t("sessionNote"),
				children: snapshot.sessions.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "us-model-meta",
					...
					className: "us-session-wrap",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("table", {
						className: "us-session-table",
						"aria-label": t("sessionRanking"),
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("tr", { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", { children: t("sessionColName") }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", { children: t("sessionColSpan") }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", {
								className: "us-number",
								children: t("sessionColDuration")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", {
								className: "us-number",
								children: t("sessionColTokens")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", {
								className: "us-number",
								children: t("sessionColMessages")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("th", { children: t("sessionColModel") })
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("td", {
									className: "us-session-name",
									title: row.cwd ?? row.sessionId,
									children: [sessionLabel(row), row.subtask && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: "us-subnote",
										children: t("sessionSubtask")
									})]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", {
									className: "us-session-span",
									title: formatExactTime(row.startTime ?? row.createdAt, numberLocale),
									children: formatCallTime(row.startTime ?? row.createdAt)
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", {
									className: "us-number",
									children: formatCallDuration(row.startTime === null || row.endTime === null ? null : row.endTime - row.startTime, t)
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", {
									className: "us-number",
									title: formatExactTokens(row.tokens, numberLocale),
									children: compact(row.tokens, numberLocale)
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", {
									className: "us-number",
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("td", {
									className: "us-session-model",
									title: row.topModel,
									children: [row.topModel.slice(row.topModel.indexOf("/") + 1), row.modelCount > 1 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
										className: "us-subnote",
										children: ["+", row.modelCount - 1]
									})]
								})
							]
						}, row.sessionId)) })]
					...
					className: "us-session-foot",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "us-model-meta",
						children: t("sessionCount", { n: snapshot.sessionTotal })
					}), snapshot.sessions.length > SESSION_ROW_LIMIT && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: "us-model-more",
						onClick: () => setExpanded((current) => !current),
						children: expanded ? t("showLess") : t("showAllSessions", { n: snapshot.sessions.length })
					})]
				})] })
			});
		}
		function Breakdown({ snapshot }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Panel, {
				id: "breakdown",
				title: t("tokenComposition"),
				note: t("cacheIncludedNote"),
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		...
		function CallsPanel({ snapshot, range, scope, workspace, session, onClearSession, custom, tick }) {
			const { t, numberLocale } = useLocale();
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Panel, {
				...
				title: t("callsTitle"),
				note: t("callsNote"),
				className: "us-panel-calls",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "us-calls-toolbar",
					children: [
						session !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							type: "button",
							className: "us-calls-chip",
							title: t("clearSessionFilter"),
							onClick: onClearSession,
							children: [t("filteringSession", { id: session.length > 18 ? `${session.slice(0, 18)}…` : session }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								"aria-hidden": "true",
								children: "×"
							})]
						}),
		...
		*/
		...
		function Dashboard({ hide, embedded = false }) {
			...
			const [custom, setCustom] = (0, react.useState)(null);
			...
			const refresh = (0, react.useCallback)((signal) => {
				setError(null);
				fetchSnapshot(range, scope, workspace, signal, custom ?? void 0).then(setSnapshot).catch((reason) => {
					if (reason.name !== "AbortError") setError(reason instanceof Error ? reason.message : String(reason));
				});
			}, [
				range,
			...
			const drillInto = (0, react.useCallback)((sessionId) => {
				setSession(sessionId);
				panels.open("calls");
				window.setTimeout(() => {
					document.querySelector(".us-panel-calls")?.scrollIntoView({
						behavior: "smooth",
						block: "start"
					});
				}, 90);
			...
			const sparkline = (0, react.useMemo)(() => {
				const days = snapshot?.days ?? [];
				if (days.length < 2) return void 0;
			...
			const windowNote = snapshot === null ? t("windowRange", {
				label: rangeLabel,
				from: "…",
				to: "…"
			}) : snapshot.range.from === "" ? t("windowAll") : t("windowRange", {
				label: rangeLabel,
			...
			const body = error ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "tokens",
							label: t("tokensUsage"),
							value: compact(snapshot.totals.tokens, numberLocale),
							detail: t("inputOutputDetail", {
								input: compact(snapshot.totals.input, numberLocale),
								output: compact(snapshot.totals.output, numberLocale)
							}),
							accent: "#1684ff",
							hero: true,
							spark: sparkline
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "chat",
							label: t("sessions"),
							value: snapshot.totals.sessions,
							detail: snapshot.totals.subagentSessions > 0 ? t("subagentNote", { n: snapshot.totals.subagentSessions }) : void 0,
							accent: "#9368ef"
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "message",
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "chart",
							label: t("calls"),
							value: compact(snapshot.models.reduce((sum, model) => sum + model.calls, 0), numberLocale),
							accent: "#22b8b5"
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "tokens",
							...
							value: (() => {
								const denom = snapshot.totals.input + snapshot.totals.cacheRead + snapshot.totals.cacheWrite;
								return denom > 0 ? `${(snapshot.totals.cacheRead / denom * 100).toFixed(1)}%` : "—";
							})(),
							accent: "#2da2bb"
						}),
						snapshot.cost !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "chart",
							label: t("cost"),
							value: `≈$${formatCost(snapshot.cost.total, numberLocale)}`,
							detail: snapshot.cost.peakShare === void 0 ? t("costCoverage", {
								percent: Math.round(snapshot.cost.pricedTokens / Math.max(1, snapshot.cost.pricedTokens + snapshot.cost.unpricedTokens) * 100),
								unpriced: compact(snapshot.cost.unpricedTokens, numberLocale)
							}) : `${t("costCoverage", {
								percent: Math.round(snapshot.cost.pricedTokens / Math.max(1, snapshot.cost.pricedTokens + snapshot.cost.unpricedTokens) * 100),
								unpriced: compact(snapshot.cost.unpricedTokens, numberLocale)
							})} · ${t("costPeakBlend", { percent: Math.round(snapshot.cost.peakShare * 100) })}`,
							accent: "#f59e0b"
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "calendar",
							label: t("activeDays"),
							value: snapshot.totals.activeDays,
							accent: "#f59e0b"
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "streak",
							label: `${t("streak")} · ${t("allTimeTag")}`,
							value: snapshot.totals.currentStreak,
							accent: "#ef5da8"
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "streak",
							label: `${t("longestStreak")} · ${t("allTimeTag")}`,
							value: snapshot.totals.longestStreak,
							accent: "#a479e2"
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "clock",
							label: t("peakHour"),
							value: formatHour(snapshot.totals.peakHour, t),
							accent: "#2da2bb"
						}),
						snapshot.mostUsedModel ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "model",
							label: t("mostUsedModel"),
							value: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: { fontSize: "18px" },
								children: snapshot.mostUsedModel.model
							}),
							detail: `${snapshot.mostUsedModel.percent.toFixed(1)}% · ${snapshot.mostUsedModel.provider}`,
							accent: "#65a9ff"
						}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Card, {
							icon: "model",
							label: t("mostUsedModel"),
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(SmoothTrend, {
					snapshot,
					note: windowNote
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ModelPanel, { snapshot }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Breakdown, { snapshot }),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(SessionRanking, {
					snapshot,
					onSelect: drillInto
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CallsPanel, {
					...
					workspace,
					session,
					onClearSession: () => setSession(""),
					custom: custom ?? void 0,
					tick
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Footer, { snapshot })
			] });
			const toolbar = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						children: [RANGE_OPTIONS.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							"aria-pressed": custom === null && range === option.value,
							onClick: () => {
								setCustom(null);
								setRange(option.value);
							},
							children: t(option.label)
						}, option.value)), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				...
				custom !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "us-custom-range",
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "us-segment us-custom-mode",
							"aria-label": t("customRange"),
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								"aria-pressed": custom.to !== void 0,
								onClick: () => setCustom((current) => current === null ? current : {
									from: current.from,
									to: current.to ?? localDate()
								}),
								children: t("customStartEnd")
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								"aria-pressed": custom.to === void 0,
								onClick: () => setCustom((current) => current === null ? current : { from: current.from }),
								children: t("customUntilNow")
							})]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							type: "date",
							value: custom.from,
							max: custom.to ?? localDate(),
							"aria-label": t("customRange"),
							onChange: (event) => {
								const value = event.target.value;
								if (value !== "") setCustom((current) => current === null ? current : {
									...current,
									from: value
								});
							}
						...
						custom.to === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "us-until-now",
							children: t("untilNow")
						}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							type: "date",
							value: custom.to,
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("a", {
							className: "us-export",
							href: exportUrl(range, scope, workspace, "csv", custom ?? void 0),
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Icon, {
								name: "download",
								size: 15
							}), "CSV"]
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("a", {
							className: "us-export",
							href: exportUrl(range, scope, workspace, "json", custom ?? void 0),
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Icon, {
								name: "download",
								size: 15
							}), "JSON"]
