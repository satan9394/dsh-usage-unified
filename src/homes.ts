...
export async function discoverDshHomes(options: DiscoveryOptions): Promise<HomeDiscovery> {
  ...
  // Every immediate `~/.dsh*` directory is a candidate home: the live one,
  // versioned Desktop shells (`.dsh_desktop/<version>`), and archived or
  // backup homes (`.dsh-backup_*`, `.dsh_daily`, …). A candidate without a
  // `sessions` directory is skipped silently, so a broad scan is safe.
  ...
  try {
    for (const entry of await readdir(osHome, { withFileTypes: true })) {
      ...
      candidates.add(home)
      if (entry.name === DESKTOP_DATA_DIR) {
        try {
          for (const version of await readdir(home, { withFileTypes: true })) {
            if (version.isDirectory()) candidates.add(join(home, version.name))
          }
        } catch (error) {
          if (errorCode(error) !== 'ENOENT') problems.push({ home, error: errorCode(error) })
        }
      }
    }
  } catch (error) {
    // A home listing failure is worth surfacing; everything else still scans.
    if (errorCode(error) !== 'ENOENT') problems.push({ home: osHome, error: errorCode(error) })
  }
