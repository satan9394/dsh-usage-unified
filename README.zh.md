...
- **「天」按宿主进程的时区切**，既不是浏览器的、也不是 UTC。折叠在建立索引时就把 `YYYY-MM-DD` 写进切片，所以时区在**索引构建那一刻**就定死了（取 DSH 进程的 `Intl.DateTimeFormat().resolvedOptions().timeZone`），改时区会触发全量重建。若机器的系统时区与使用者所在时区不同，「今天」指的就是**那个时区**的今天——想让面板跟随指定时区，就给 DSH 进程设 `TZ`。
